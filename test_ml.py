...
def test_inference_shape_value():
    """
    # Testing that the inference function returns predictions of correct shape and values, 0/1.
    """
    # Your code here
    model = train_model(X_train, y_train)
    preds = inference(model, X_test)
    assert len(preds) == len(y_test), "Inference output shape does not match expected shape"
